...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/0cash/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/0cash/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    # Assume that if 0cash is an ELF binary, they all are
    with open(repofile('src/0cashd'), 'rb') as f:
        magic = f.read(4)
        if not magic.startswith(b'\x7fELF'):
            return ret

    ret &= test_rpath_runpath('src/0cashd')
    ret &= test_rpath_runpath('src/0cash-cli')
    ret &= test_rpath_runpath('src/0cash-gtest')
    ret &= test_rpath_runpath('src/0cash-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/0cashd')
    ret &= test_fortify_source('src/0cash-cli')
    ret &= test_fortify_source('src/0cash-gtest')
    ret &= test_fortify_source('src/0cash-tx')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/0cash-gtest')],
