...
PACKAGE_VERSION=$($SRC_PATH/src/0cashd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/0cashd $DEB_BIN
cp $SRC_PATH/src/0cash-cli $DEB_BIN
...
# Copy manpages
cp $SRC_DOC/man/0cashd.1 $DEB_MAN
cp $SRC_DOC/man/0cash-cli.1 $DEB_MAN
cp $SRC_DOC/man/zcash-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/0cashd.bash-completion $DEB_CMP/0cashd
cp $SRC_PATH/contrib/0cash-cli.bash-completion $DEB_CMP/0cash-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/0cashd.1
gzip --best -n $DEB_MAN/0cash-cli.1
gzip --best -n $DEB_MAN/zcash-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/0cashd $DEB_BIN/0cash-cli
